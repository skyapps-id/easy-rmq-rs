...
impl Publisher {
    ...
    pub fn with_trace_id(self, trace_id: String) -> PublisherWithTrace {
        PublisherWithTrace {
            publisher: self,
    ...
    pub fn with_auto_trace_id(self) -> PublisherWithTrace {
        PublisherWithTrace {
            publisher: self,
            trace_id: Some(crate::generate_trace_id()),
        }
    }
...
impl PublisherWithTrace {
    pub async fn publish_json<T: serde::Serialize>(
        ...
        let json = serde_json::to_vec(payload).map_err(AmqpError::SerializationError)?;
        self.publish(&self.publisher.exchange, routing_key, &json).await
    }
    ...
    pub async fn publish_text(&self, routing_key: &str, payload: &str) -> Result<()> {
        self.publish(&self.publisher.exchange, routing_key, payload.as_bytes())
            .await
    }
    ...
    async fn publish(&self, exchange: &str, routing_key: &str, payload: &[u8]) -> Result<()> {
        let channel = self.publisher.channel_pool.get_channel().await?;
